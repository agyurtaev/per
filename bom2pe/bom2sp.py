...
## Второй проход: Работа с неустановленными
    ...
    ofile  = open('projectname_tdd_sp_1.csv', "wb")
...
## Второй проход: Удаление контрольных точек
    ...
    ifile  = open('projectname_tdd_sp_1.csv', "rb")
    readerd.__init__(ifile, delimiter=";", quoting=csv.QUOTE_NONE)
    ofile  = open('projectname_tdd_sp_2.csv', "wb")
    writerd = csv.DictWriter(ofile, delimiter=';',fieldnames=header, quoting=csv.QUOTE_NONE)
    writerd.writeheader()
...
## Третий проход: Работа с PartNumberRU и PartDocument
    ...
    ifile  = open('projectname_tdd_sp_2.csv', "rb")
    readerd.__init__(ifile, delimiter=";", quoting=csv.QUOTE_NONE)
    ofile  = open('projectname_tdd_sp_3.csv', "wb")
    writerd = csv.DictWriter(ofile, delimiter=';',fieldnames=header, quoting=csv.QUOTE_NONE)
    writerd.writeheader()
...
## Четвёртый проход: Работа с PartDocument
    ...
    ifile  = open('projectname_tdd_sp_3.csv', "rb")
    readerd.__init__(ifile, delimiter=";", quoting=csv.QUOTE_NONE)
    ofile  = open('projectname_tdd_sp_4.csv', "wb")
    writerd = csv.DictWriter(ofile, delimiter=';',fieldnames=header, quoting=csv.QUOTE_NONE)
    writerd.writeheader()
...
## Четвертый проход: Работа с допустимыми заменами

    ifile  = open('projectname_tdd_sp_4.csv', "rb")
    readerd.__init__(ifile, delimiter=";", quoting=csv.QUOTE_NONE)
    ofile  = open('projectname_tdd_sp_5.csv', "wb")
    writerd = csv.DictWriter(ofile, delimiter=';',fieldnames=header, quoting=csv.QUOTE_NONE)
    writerd.writeheader()
