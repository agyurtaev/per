...
#################################- прочие изделия; /Poz;Name;Kol
    NUM = _priz_bom_sort_isp.prizgen(NUM,perecod,NUM_sbed,NUM_det,NUM_stiz,unpl_spis)
...
#################################- редактирование latex;
    ...
    copyfile('partn_old.txt','partn_old1.txt')
    
    ofile = open('partn_old.txt', 'w')
    
    ifile  = open('partn_old1.txt', "rb")
    for row in ifile:
        ofile.write(row)
    ifile.close()
    
    ifile  = open('partn_new.txt', "rb")
    for row in ifile:
        ofile.write(row)
    ifile.close()
